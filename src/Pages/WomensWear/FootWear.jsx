...
const WomenFootWear = () => {
    ...
    const urlforFoot = "https://get-styled-backend.onrender.com/product/getAllProduct";
    ...
    const getFoot = async() =>{
        try {
            ...
            const filteredData = res.data.result.filter(item => item.ProductType === "Shoes");
            const filteredDataforMen = filteredData.filter(item => item.ProductCategory === "Women");
            setFootWear(filteredDataforMen);
          } catch (err) {
